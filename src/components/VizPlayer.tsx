...
/**
 * Plays a lesson's step-script: 80 scripts share 11 render families.
 * Frames may carry a predict challenge {q,a,b,t} — playback pauses there
 * until the learner commits an answer.
 */
  ...
  return (
    <figure className="blueprint" style={{ margin: 0, background: "var(--color-bg)" }}>
      ...
      <div style={{ padding: "var(--space-4) var(--space-6)", borderBottom: "1px solid var(--color-divider)", display: "flex", alignItems: "baseline", gap: "var(--space-3)" }}>
        <span className="kicker">Visualizer</span>
...
function FamilyRenderer({ family, frame }: { family: string; frame: Frame }) {
  switch (family) {
    ...
    case "objects": return <Objects f={frame} />;
...
function Bars({ f }: { f: Frame }) {
  ...
  const act = !!f.act;
  const pivot = f.pivot as number | null;
...
function Cells({ f }: { f: Frame }) {
  const cells = (f.cells as { v: unknown; i?: number; tag?: string; s?: string }[]) ?? [];
  return (
        ...
        <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
          ...
          <div
            style={{
              ...
              background: c.s === "hot" ? "var(--color-accent)" : "var(--color-bg)",
    ...
    <div style={{ display: "flex", gap: 6, justifyContent: "center", alignItems: "flex-start" }}>
      {buckets.map((b, i) => (
        <div key={i} style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "center", minWidth: 62 }}>
          ...
          >
            {b.map((k, j) => (
              <span key={j} style={{ background: "color-mix(in srgb, var(--color-accent) 18%, transparent)", fontSize: 11, textAlign: "center", padding: "3px 2px" }}>
                {k}
              </span>
            ))}
          </div>
...
function Graph({ f }: { f: Frame }) {
  ...
  return (
    <svg viewBox={`0 0 ${maxX} ${maxY}`} style={{ width: "100%", maxHeight: 280 }}>
      {edges.map(({ a, b, w }, i) => {
...
function Objects({ f }: { f: Frame }) {
  const cls = f.cls as { name: string; fields: string[] } | undefined;
  const objs = (f.objs as { id: number; type: string; colour: string }[]) ?? [];
  const hot = f.hot as number;
  return (
    <div style={{ display: "flex", gap: "var(--space-6)", justifyContent: "center", alignItems: "flex-start", flexWrap: "wrap" }}>
      {cls && (
        <div style={{ ...box, fontFamily: "inherit", padding: "var(--space-3)", borderStyle: "dashed", borderColor: f.clsHot ? "var(--color-accent)" : "var(--color-divider)", minWidth: 200 }}>
          <div className="kicker">class {cls.name}</div>
          {cls.fields.map((fd) => (
            <div key={fd} className="mono" style={{ fontSize: 12, padding: "3px 0", color: "var(--color-neutral-800)" }}>{fd}</div>
          ))}
          <div className="text-muted" style={{ fontSize: 10, marginTop: 6, fontStyle: "italic" }}>the drawing — nobody lives here</div>
        </div>
      )}
      <div style={{ display: "flex", gap: "var(--space-3)", flexWrap: "wrap" }}>
        {objs.length === 0 && <span className="text-muted" style={{ fontSize: 12, alignSelf: "center" }}>no objects yet</span>}
        {objs.map((o, i) => (
          <div key={o.id} style={{ ...box, fontFamily: "inherit", padding: "var(--space-3)", minWidth: 120, borderColor: i === hot ? "var(--color-accent)" : "var(--color-divider)", borderWidth: i === hot ? 2 : 1 }}>
            <div className="mono" style={{ fontSize: 11, color: "var(--color-accent-700)" }}>{o.type.toLowerCase()}{o.id}</div>
            <div className="mono" style={{ fontSize: 12 }}>colour: {o.colour}</div>
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", maxHeight: 240 }}>
        ...
        <text x={W - 10} y={H - 8} textAnchor="end" fontSize="10" fill="var(--color-neutral-600)">n = {n}</text>
        <text x={10} y={16} fontSize="10" fill="var(--color-neutral-600)">steps (log scale)</text>
        {series.map((s, si) => {
          let d = "";
          for (let x = 1; x <= n; x++) d += `${x === 1 ? "M" : "L"}${px(x).toFixed(1)},${py(val(s, x)).toFixed(1)}`;
